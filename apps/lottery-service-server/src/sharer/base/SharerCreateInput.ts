/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  IsOptional,
  ValidateNested,
  MaxLength,
  IsInt,
  Min,
  Max,
} from "class-validator";
import { LotteryWhereUniqueInput } from "../../lottery/base/LotteryWhereUniqueInput";
import { Type } from "class-transformer";
import { ShareRecordCreateNestedManyWithoutSharersInput } from "./ShareRecordCreateNestedManyWithoutSharersInput";

@InputType()
class SharerCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: () => LotteryWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LotteryWhereUniqueInput)
  @IsOptional()
  @Field(() => LotteryWhereUniqueInput, {
    nullable: true,
  })
  lottery?: LotteryWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  points?: number | null;

  @ApiProperty({
    required: false,
    type: () => ShareRecordCreateNestedManyWithoutSharersInput,
  })
  @ValidateNested()
  @Type(() => ShareRecordCreateNestedManyWithoutSharersInput)
  @IsOptional()
  @Field(() => ShareRecordCreateNestedManyWithoutSharersInput, {
    nullable: true,
  })
  shareRecords?: ShareRecordCreateNestedManyWithoutSharersInput;
}

export { SharerCreateInput as SharerCreateInput };
